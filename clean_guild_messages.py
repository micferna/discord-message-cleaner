import requests
import time
from utils import DISCORD_TOKEN, USER_ID, EXCLUDED_GUILD_IDS, print_color
...
def get_guilds():
    ...
    if response.status_code == 200:
        ...
        print_color(f"Erreur lors de la récupération des guildes: {response.status_code}", "red")
        print(response.text)
        return []
...
def main():
    ...
    for guild in guilds:
        if guild['id'] in EXCLUDED_GUILD_IDS:
            print_color(f"Saut de la guilde {guild['name']} ({guild['id']})", "cyan")
            continue
        print_color(f"Traitement des canaux dans la guilde {guild['name']} ({guild['id']})", "cyan")
